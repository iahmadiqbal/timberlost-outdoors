...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-gradient-to-br from-primary-900 via-primary-800 to-accent-800 shadow-2xl transform transition-all duration-700 ease-out z-50 ${
        isOpen ? 'translate-x-0' : 'translate-x-full'
      }`}>
        {/* Decorative Elements */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-10 right-10 w-32 h-32 bg-accent-500/20 rounded-full blur-3xl animate-pulse"></div>
          <div className="absolute bottom-20 left-10 w-40 h-40 bg-primary-500/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        </div>

        {/* Close Button */}
        <div className="relative flex justify-between items-center p-6 border-b border-white/20 backdrop-blur-sm">
          <div className="flex items-center space-x-3">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-br from-accent-400 to-accent-600 rounded-xl blur-md opacity-50"></div>
              <div className="relative w-12 h-12 bg-gradient-to-br from-accent-400 to-accent-600 rounded-xl flex items-center justify-center shadow-xl">
                <span className="text-2xl font-black text-white">T</span>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-black text-white">Menu</h3>
              <p className="text-xs text-accent-300 font-semibold">Navigation</p>
            </div>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="w-12 h-12 bg-white/10 hover:bg-white/20 rounded-xl flex items-center justify-center transition-all duration-300 transform hover:rotate-90 hover:scale-110 shadow-lg"
          >
        ...
        <div className="relative px-6 py-8 space-y-3 overflow-y-auto h-[calc(100%-180px)]">
          {navLinks.map((link, index) => (
            <Link
              ...
              to={link.path}
              onClick={() => setIsOpen(false)}
              className={`block px-6 py-4 rounded-2xl font-bold text-lg transition-all duration-300 transform hover:scale-105 hover:translate-x-2 ${
                isActive(link.path)
                  ? 'bg-gradient-to-r from-accent-500 to-accent-600 text-white shadow-2xl'
                  : 'text-white hover:bg-white/20 border-2 border-white/10 hover:border-white/30'
              } ${isOpen ? 'slide-in-right' : ''}`}
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="flex items-center justify-between">
                <span className="flex items-center space-x-3">
                  <span className={`w-2 h-2 rounded-full ${isActive(link.path) ? 'bg-white' : 'bg-accent-400'}`}></span>
                  <span>{link.name}</span>
                </span>
                <svg className={`w-5 h-5 transform transition-transform ${isActive(link.path) ? 'translate-x-1' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative px-6 pb-6">
          <Link
            to="/contact"
            onClick={() => setIsOpen(false)}
            className={`block px-6 py-5 bg-white text-primary-900 rounded-2xl font-black text-lg text-center transition-all duration-300 transform hover:scale-105 shadow-2xl hover:shadow-accent-500/50 ${isOpen ? 'slide-in-right' : ''}`}
